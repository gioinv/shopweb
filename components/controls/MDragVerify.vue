<template>
  <div
    ...
    @touchend="dragFinish"
  >
    <div ref="progressBar" class="dv_progress_bar" :style="progressBarStyle" />
    <div ref="message" class="dv_text" :style="textStyle">
      {{ message }}
    </div>

    <div ref="handler" class="dv_handler dv_handler_bg" :style="handlerStyle" @mousedown="dragStart" @touchstart="dragStart">
      <i :class="handlerIcon" />
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    init () {
      ...
      this.$refs.handler.className += ' dv_handler_ok_bg'
      this.$refs.handler.children[0].className = this.handlerIcon
